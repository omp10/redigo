    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#F8FAFC_0%,#F3F4F6_38%,#EEF2F7_100%)] max-w-lg mx-auto font-sans pb-28 relative overflow-hidden">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 gap-3">
          <span className="w-8 h-8 border-2 border-slate-900 border-t-transparent rounded-full animate-spin" />
          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Loading seat layout...</p>
        </div>
      ) : (
        <div className="px-5 pt-4 space-y-4">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                  ...
                  {row.slice(0, 2).map(seat => seat ? (
                    <motion.button key={seat.id} whileTap={seat.status !== 'booked' && seat.status !== 'gap' ? { scale: 0.9 } : {}}
                      ...
                      className={`w-14 h-14 rounded-[14px] border-2 flex flex-col items-center justify-center gap-0.5 transition-all ${
                        ...
                        seat.status === 'selected'  ? 'bg-slate-900 border-slate-900 shadow-[0_4px_12px_rgba(15,23,42,0.2)] text-white' :
            ...
            <div className="flex justify-center gap-4 pt-2 border-t border-slate-50">
              {[['bg-white border-slate-200','Available'],['bg-slate-900 border-slate-900','Selected'],['bg-slate-100 border-slate-100','Booked']].map(([cls,lbl]) => (
